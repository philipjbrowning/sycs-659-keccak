...
bool TrailCore3Rounds::canAffordExtendingChain() const
{
    ...
    if (chains.back().size() >= minimumWorkingChainLength) {
        ...
        nrOrbitalPointsPerDeltaChain = (unsigned int)chains.back().size() - chainOdd; // If the working chain has an odd number of points, it contains (its size minus 1) orbital points. If the working chain has an even number of points, it currently contains (its size minus 2) orbital points, which becomes (its size) after extension.
    }
    else {
        deltaNrOrbitalPointsWorkingChain = minimumWorkingChainLength - (unsigned int)chains.back().size() - 1; // After extension, the working chain will contain at least (its current size minus 1) orbital points. The working chain has to reach at least (minimumWorkingChainLength - 2) orbital points.
...
void TrailCore3Rounds::updateMinimumWorkingChainLength()
{
    if (chains.size() <= 1)
        ...
        minimumWorkingChainLength = (unsigned int)chains[chains.size()-2].size();
        ...
        if ((chainAtB.size()%2) == 0) { // The next point to add has to be chained to the last one (i.e., in the same column at B).
            ...
            if ( slicesAtB.count(pB.z) == 0 ) {  // if the point arrives in an empty slice, add it
                ...
                if (2*chainAtB.size() + 2*rowsAtA.size() + 2*rowsAtD.size() <= maxWeight) {
                    addVortexToBaseIfMinimal(chainAtB,(unsigned int)rowsAtA.size(),(unsigned int)rowsAtD.size());
                }
